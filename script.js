...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tg.ready();
    tg.expand();
    tg.setHeaderColor('#211A15');
    tg.setBackgroundColor('#120F0C');
    ...
    const contactLinks = [
        {
            name: 'TELEGRAM ✈️',
            url: 'https://t.me/LVR741',
            id: 'telegram-main',
            className: 'telegram',
            text: 'TELEGRAM LE BARA ✈️'
        },
        {
            name: 'WHATSAPP 📞',
            url: 'https://wa.me/33676196864',
            id: 'whatsapp',
            className: 'whatsapp',
            text: 'WHATSAPP 📞'
        }
    ...
    // --- DONNÉES DE L'APPLICATION (LE BARA) ---
    const appData = [
        {
            id: 'MACH',
            name: 'MACH 🚀',
    ...
    function renderHomePage() {
        ...
        if (currentView === 'categories') {
            ...
            align-items: center;
            background: linear-gradient(182deg, var(--tertiary-bg-color), transparent);
                        border-radius: 10px;`;
...
function renderProductListSimple(categoryId) {
  ...
  function renderConfirmation() {
    ...
    if(contactBtn) {
        ...
        contactBtn.innerHTML = 'CONTACTER SUR WHATSAPP 📞'; // Change le texte
    }
  ...
  function formatOrderMessage() {
    ...
    // 1. Bouton vers le canal Telegram Le Bara
    ...
    if (btnChannel) {
        btnChannel.addEventListener('click', function() {
            ...
            const channelLink = "https://t.me/LVR741"; 
    ...
    document.body.addEventListener('click', async function (e) {
        ...
        if (target.closest('.pack-item-btn')) {
          ...
          // Clic sur "Confirmer la commande" (WHATSAPP LE BARA)
    if (target.closest('#confirm-order-button')) {

        // Numéro WhatsApp Le Bara au format international, sans le +
        const myPhoneNumber = '33676196864';

        // Génère le récapitulatif complet du panier
        const message = formatOrderMessage();
        const encodedMessage = encodeURIComponent(message);

        // Ouvre WhatsApp avec la commande déjà préremplie
